...
The root CA certifcate and wildcard may not be in the same namespace of the keycloak. So you may need to sync/copy them from other namespace. You can use this [doc](./sys_admin/11.Copy_secret_between_name_space.md) as a guid. 

## 6.3 Keycloak Installation

We use the codecentric Helm chart to deploy a keycloak instance, with the provided configuration in [keycloak.yaml](../resources/keycloak/keycloak.yaml). Below are the commands:
