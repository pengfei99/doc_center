...
The global hadoop root dir ownership and permission setup:
```shell
sudo chown -R root:hadoop -R /opt/hadoop
sudo chmod -R 750 /opt/hadoop

# to allow hadoop group to edit the conf files
# 
sudo chmod -R 770 $HADOOP_HOME/etc/hadoop
# if you did not change the default log path, and you run your hadoop daemons as hadoop
...
The `container-executor` executor owners and permission setup:
...
### 1.4 Redirect container workspace

By default, yarn write the container log in `$HADOOP_HOME/logs/userlogs`. As the container runs with the uid of the
user who submit the job, the container does not have the right to write logs here.

As a result, we need to redirect the logs to another folder which everyone has right to write
```shell
sudo mkdir -p /var/log/hadoop-yarn/containers
sudo chown -R hadoop:hadoop /var/log/hadoop-yarn
# the 1 is the sticky bit that allows multiple users to write safely, like /tmp
# Each container gets isolated subdirectories.
sudo chmod -R 1777 /var/log/hadoop-yarn/containers
```

Then you need to add the below lines to the `yarn-site.xml` 
```xml
<property>
...
### 1.5 Specify a local container workspace

To prevent future permission explosions, we need to fix the local container workspace

```shell
sudo mkdir -p /var/lib/hadoop-yarn/local
sudo chown -R hadoop:hadoop /var/lib/hadoop-yarn
sudo chmod -R 1777 /var/lib/hadoop-yarn/local
...
With the above config now all your `/opt/hadoop` folder has `750`, which means if you are not root or not in hadoop group
you can not read the core-site.xml, yarn-site.xml in `$HADOOP_HOME/etc/hadoop/`. When users submit a spark job, the RM
will ask worker node to create containers, these processes run with the access right of the user who submit the spark job.
If this user is:
- not root (not even in his dream)
- not in group hadoop (99% of the case)

These process can not read any config file(e.g. core-site.xml, yarn-site.xml) in `$HADOOP_HOME/etc/hadoop/`. It means 
the JavaSparkContext cannot read the cluster topology and will crash during the handshake. As a result, the spark job
will fail. 

To overcome this solution, we can add a secondary group(e.g. spark-users) with rx right to `/opt/hadoop`.

```shell
# install the setfacl module
sudo apt update
sudo apt install acl -y

# add secondary group
sudo setfacl -R -m g:spark-users:rx /opt/hadoop/
```
