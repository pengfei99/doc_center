...
```shell
# create group
sudo addgroup --system gmsa

# create service account
sudo adduser --system --no-create-home --shell=/usr/sbin/nologin --ingroup=gmsa gmsa
...
The `pip install --break-system-packages gmsad` may failed, because it requires some system dependencies 
to build the binary. You can install the below libs
...
# Configuration gMSA
gmsa_name = spark4-03
gmsa_dn = CN=spark4-03,CN=Managed Service Accounts,DC=casdds,DC=casd
gMSA_sAMAccountName = spark4-03$
gMSA_keytab = /etc/krb5.keytab

gMSA_servicePrincipalNames = host/spark4-03.casdds.casd,HTTP/spark4-03.casdds.casd,hdfs/spark4-03.casdds.casd,yarn/spark4-03.casdds.casd,spark/spark4-03.casdds.casd
# this line is important, without it the generated .keybab will not contain all the information for the server to get ticket
gMSA_upn_in_keytab = yes
# Où écrire le keytab
target_keytab = /etc/krb5.keytab
...
```shell
sudo chown gmsa:root /etc/security/keytabs/Linux.keytab
sudo chmod 640 /etc/security/keytabs/Linux.keytab

# it may not exit for the first time, so create the file
touch /etc/krb5.keytab
sudo chown root:gmsa /etc/krb5.keytab
sudo chmod 660 /etc/krb5.keytab
```
