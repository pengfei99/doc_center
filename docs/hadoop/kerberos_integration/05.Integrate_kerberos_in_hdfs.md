...
> In the data node, you only need to specify the name node principal, you don't need the corresponding `.keytab` file.
> 
## 9. Improve log management

By default, hadoop write all logs under `$HADOOP_HOME/logs`, which is not optimal. Continue with this [tutorial]() to improve 
your log management in hdfs and yarn.