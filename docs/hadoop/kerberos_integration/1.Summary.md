...
New-ADServiceAccount -Name "gMSA-hclient" -DNSHostName "gMSA-hclient.casdds.casd"

# setup a policy to allow another account to retrive password
Set-ADServiceAccount `
  -Identity "gMSA-hclient" `
  -PrincipalsAllowedToRetrieveManagedPassword "Linux$" `
  -Verbose
```
...
### 2.4.4 Create a systemd serice for gmsad
...
[Service]
Type=simple
Environment=PATH=/opt/gmsad_venv/bin:/usr/local/bin:/usr/bin:/bin
ExecStartPre=/bin/bash -c "if [ -f /etc/krb5.keytabs/gMSA-01.keytab ]; then echo 'Suppression de l ancien keytab...'; rm -f /etc/krb5.keytabs/gMSA-01.keytab; fi"
