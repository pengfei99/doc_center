...
New-ADServiceAccount `
  ...
  -OtherAttributes @{userPrincipalName = "hdfs/deb13-spark1.casdds.casd@CASDDS.CASD"} `
  -PrincipalsAllowedToRetrieveManagedPassword "gmsa-hadoop-hosts" `
  -KerberosEncryptionType AES256,AES128
```
> This command create a gmsa account with a special Attribute called `userPrincipalName`. The value must be the complete
> and valid Kerberos principal name <name>@<RELM>, in our case `hdfs/deb13-spark1.casdds.casd@CASDDS.CASD`
...
```
> unlike the userPrincipalName, the servicePrincipalName does not require the @<RElM>
...
> The result is that after user ssh login with kerberos ticket, on linux server, you will not have any ticket. Because 
> KDC does not allow ticket delegation. So we need to change this value to True.
