...
10.50.5.80	    spark4-01.casd.local	spark4-01
10.50.5.81	    spark4-02.casd.local	spark4-02
10.50.5.82	    spark4-03.casd.local	spark4-03
...
```shell
# create a group called spark with gid 2001
sudo groupadd -g 59001 spark

# create a user account spark, the home folder is /home/spark, the default shell is bash
sudo useradd -g spark -u 2001  -r spark -m -d /home/spark --shell /bin/bash

# create a service account for hdfs, it has no home and no login shell
sudo useradd -u 2002 -g spark -r -M -s /sbin/nologin hdfs


# login as spark
sudo su -l spark

# add spark group to an existing user
...
# make sure the owner of the unzip files are root:spark
sudo chown -R root:spark /opt/hadoop/
...
export JAVA_HOME=/opt/java/jdk21.0.10
