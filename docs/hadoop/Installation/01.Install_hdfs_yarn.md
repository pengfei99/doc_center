...
Edit the conf file `core-site.xml`, and add the following lines
```shell
...
`yarn-site.xml` on Master Node (ResourceManager settings)
...
export HADOOP_SECURE_PID_DIR=/grid/0/var/run/hadoop/$HADOOP_SECURE_DN_USER
