...
spark-submit --deploy-mode cluster --class org.apache.spark.examples.SparkPi $SPARK_HOME/examples/jars/spark-examples_2.13-4.1.1.jar 10
...
```shell
# spark submit needs to write some temporary files on hdfs.
# we must provide --conf spark.yarn.stagingDir=hdfs:///users where we can find the home dir of the user.

spark-submit --deploy-mode client --conf spark.yarn.stagingDir=hdfs:///users --class org.apache.spark.examples.SparkPi $SPARK_HOME/examples/jars/spark-examples_2.12-3.4.1.jar 10
