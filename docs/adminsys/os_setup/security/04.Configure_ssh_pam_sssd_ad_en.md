...
1. Check ssh server side
2. Check ssh client side

### ssh server side(debian 13)
...
- Validate the host server keytab `/etc/krb5.keytab`
- Validate the kerberos Auth

#### Test AD user resolution



### ssh client side (Windows server)

- check user's krb ticket
- check ssh client config
- try to login via ssh

### On both sides

- check time sync: Use date to show time on both servers, the time difference must be `smaller than 5s`.
- check dns resolution : From Windows, nslookup debian.example.com resolves to Debian IP. Reverse DNS too (nslookup <IP> should give FQDN).
- check ssh server spn: With `setspn -L debian$` command, we should see `host/debian.example.com@EXAMPLE.COM`
- Test service ticket from Debian: After `kinit user@EXAMPLE.COM`, and `kvno host/debian.example.com@EXAMPLE.COM`. You 
                                 should get a number without errors.
